...
struct CanvasView: UIViewRepresentable {
    ...
    init(canvasManager: CanvasManager, size: CGSize, canvasFrame: CGRect) {
...
//        let hoverGesture = UIHoverGestureRecognizer(target: context.coordinator, action: #selector(Coordinator.handleHover(sender:)))
//        view.addGestureRecognizer(hoverGesture)
    ...
    func updateUIView(_ uiView: UIView, context: Context) {
        DispatchQueue.main.async {
            if let scrollView = uiView as? UIScrollView, let container = scrollView.subviews.first, let canvas = container.subviews.first(where: { $0 is MTKCanvas }) as? MTKCanvas {
                ...
                if let iov = container.subviews.first(where: { type(of:$0) == _UIHostingView<InteractiveOverlayView>.self }) {
    ...
    class Coordinator: NSObject, UIScrollViewDelegate, UIPencilInteractionDelegate {
        ...
        {
            print(sender.zOffset, sender.altitudeAngle)
        }
        
        func pencilInteractionDidTap(_ interaction: UIPencilInteraction) {
            switch UIPencilInteraction.preferredTapAction {
            case .ignore:
                print("Apple Pencil Double-Tap Ignore")
            case .showColorPalette:
                ControlsViewModel.shared.showColorPicker()
            case .showInkAttributes:
                ControlsViewModel.shared.showBrushLibrary()
            case .switchEraser:
                if self.parent?.canvasManager.currentMode == .brush {
                    self.parent?.canvasManager.useEraser()
                } else {
                    self.parent?.canvasManager.useBrush()
                }
            case .switchPrevious:
                self.parent?.canvasManager.usePreviousTool()
            default:
                print("Apple Pencil Double-Tap Ignore")
            }
        }
...
struct InteractiveCanvas: View {
    ...
    var body: some View {
        GeometryReader { geo in
            CanvasView(canvasManager: canvasManager, size: geo.size, canvasFrame: CGRect(x: 0, y: 0, width: size.width, height: size.height))
